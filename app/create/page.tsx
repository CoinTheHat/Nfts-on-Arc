                                ...
                                <div className="bg-surface-1 border border-border-default rounded-2xl p-6 shadow-sm">
                                    <div className="flex justify-between items-center mb-6">
                                        ...
                                        <Button
                                            ...
                                            onClick={async () => {
                                                const themes = ["Cyberpunk City", "Abstract Waves", "Retro Pixel", "Ethereal Dream"];
                                                const randomTheme = themes[Math.floor(Math.random() * themes.length)];
                                                // 1. Set Loading (Simulate AI thinking)
                                                // In a real app we'd likely have a loading state variable for this button specifically

                                                // 2. Fill Data
                                                setFormData({
                                                    ...formData,
                                                    name: `${randomTheme} Collective`,
                                                    symbol: randomTheme.split(" ").map(w => w[0]).join("") + "X",
                                                    description: `A unique collection generated by Arc AI, featuring ${randomTheme.toLowerCase()} aesthetics. Created autonomously.`,
                                                    price: "0.05",
                                                    supply: "100"
                                                });

                                                // 3. Mock Image (Use a placeholder service color)
                                                setPreviewUrl(`https://placehold.co/400x400/1e1e2e/FFF?text=${randomTheme.replace(" ", "+")}`);
                                            }}
                                        >
                                            ✨ AI Generate
                                        </Button>
